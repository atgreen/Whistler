  ...
  (whistler/loader:with-bpf-session ()
    ...
    (bpf:prog ffi_call_tracker (:type :kprobe
      ...
      (let ((cif (make-ffi-cif))
        ...
        (setf (stats-key-rtype key) (cast u8 (ffi-type-type-code ft))
              (stats-key-abi key)   (cast u8 (ffi-cif-abi cif))
              (stats-key-nargs key) (cast u16 (ffi-cif-nargs cif)))
        ...
        (do-user-ptrs (atype-ptr (ffi-cif-arg-types cif) (ffi-cif-nargs cif)
          ...
          (setf (stats-key-arg-types key i)
                (cast u8 (ffi-type-type-code ft))))
