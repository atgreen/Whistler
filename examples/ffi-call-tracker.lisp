...
;;; whistler:defstruct generates both BPF macros and CL struct + decoder.

(whistler:defstruct ffi-cif
  (abi u32) (nargs u32) (arg-types u64) (rtype u64))

(whistler:defstruct ffi-type
...
;;; ========== Stats formatting ==========
;;; decode-stats-key is auto-generated by whistler:defstruct.
;;; It returns a stats-key-record CL struct.

(defun format-stats-key (key-bytes)
  "Format a stats-key byte array into (comm signature) strings."
  (let* ((key (decode-stats-key key-bytes))
         (comm-bytes (stats-key-record-comm key))
         (comm-end (or (position 0 comm-bytes) (length comm-bytes)))
         (comm (map 'string #'code-char (subseq comm-bytes 0 comm-end)))
         (nargs (stats-key-record-nargs key))
         (rtype (stats-key-record-rtype key))
         (abi (stats-key-record-abi key))
         (arg-types (stats-key-record-arg-types key))
         (n (min nargs 16))
         (args (with-output-to-string (s)
                 (dotimes (i n)
                   (when (plusp i) (write-string ", " s))
                   (write-string (ffi-type-name (aref arg-types i)) s))
                 (when (> nargs 16)
              ...
              (loop
                (let ((next-key (map-get-next-key stats-map key)))
                  ...
                  (let ((value (map-lookup stats-map next-key)))
                    (when value
                      (multiple-value-bind (comm sig) (format-stats-key next-key)
