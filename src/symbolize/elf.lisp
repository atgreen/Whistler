...
(defun read-section-headers (buf)
  ...
  (multiple-value-bind (e-shoff e-shentsize e-shnum e-shstrndx)
      (elf64-shdr-table buf)
    (let ((secs (make-array e-shnum)))
      (dotimes (i e-shnum)
        (let ((off (+ e-shoff (* i e-shentsize))))
          (multiple-value-bind (name-off type flags sec-off size link)
              (elf64-shdr-fields buf off)
            (declare (ignore flags))
            (setf (aref secs i)
                  (make-section :name-off name-off
                                :type     type
                                :offset   sec-off
                                :size     size
                                :link     link)))))
...
(defun parse-symbol-table (buf sym-sec str-sec)
  ...
  (let* ((base      (section-offset sym-sec))
    ...
    (dotimes (i n)
      (multiple-value-bind (name-off info shndx st-val st-sz)
          (elf64-sym-fields buf (+ base (* i 24)))
        (declare (ignore shndx))
        (when (and (= (logand info #x0f) +stt-func+) (plusp st-sz))
          (let ((name (cstring-at buf (+ str-base name-off))))
